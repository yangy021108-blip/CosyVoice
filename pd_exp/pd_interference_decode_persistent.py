#!/usr/bin/env python3
"""Persistent PD decode worker for low-latency interference measurements."""
...
def parse_args():
    ...
    p.add_argument("--repeats",type=int,default=30); p.add_argument("--side-channel-port",type=int,required=True)
    p.add_argument("--barrier-socket",required=True); p.add_argument("--timeout",type=float,default=900)
...
def main():
    ...
    if a.instrument_nixl:
        os.environ["COSY_PD_INSTRUMENT_NIXL"]="1"; os.environ["COSY_PD_TRACE_FILE"]=str(a.work/"decode_trace.jsonl")
    if a.graph_safe_remote_prefill: os.environ["COSY_PD_GRAPH_SAFE_REMOTE_PREFILL"]="1"
    ...
    try:
        ...
        def run_decode(name,req_id,tag,measured=False):
            wait_for_file(a.work/f"{tag}_prefill.json",a.timeout)
            pre=read_json(a.work/f"{tag}_prefill.json")
            ...
            if measured:
                os.environ["COSY_PD_PROGRESS_SOCKET"]=a.barrier_socket+"_"+tag.replace("measured_", "", 1)+".sock"
                os.environ["COSY_PD_PROGRESS_FILE"]=str(a.work/f"token40_{tag}.json")
                os.environ["COSY_PD_PROGRESS_REQUEST_ID"]=req_id; os.environ["COSY_PD_PROGRESS_TOKENS"]="40"
                result=run_engine_requests(engine,[{"request_id":req_id,"prompt_embeds":prompt,"sampling_params":sampling}],
                ...
                tokens,metrics,transfer=run_engine_request(engine,prompt,sampling,request_id=req_id,
                                                            timeout_seconds=a.timeout,nvtx_label="COSY_PD_"+name.upper())
            ...
            write_json(a.work/f"{name}_decode.json",{"status":"ok","request_id":req_id,
                ...
                "raw_speech_tokens":tokens,"metrics":metrics,"kv_transfer_params":transfer})
        ...
        for r in range(a.repeats):
            tag=f"r{r:02d}"
            run_decode("control_"+tag,"pd-d-control-"+tag,"control_"+tag,False)
            run_decode("measured_"+tag,"pd-d-measured-"+tag,"measured_"+tag,True)
        return 0
    except Exception as exc:
        err={"status":"error","stage":"decode","exception":f"{type(exc).__name__}: {exc}",
             "traceback":traceback.format_exc()}
        for path in [a.work/"warm_decode.json"]+[a.work/f"{n}_r{r:02d}_decode.json" for r in range(a.repeats) for n in ("control","measured")]:
            if not path.exists(): write_json(path,err)
        for r in range(a.repeats):
            write_json(a.work/f"control_r{r:02d}_release.json",{"status":"decode_failed"})
            write_json(a.work/f"measured_r{r:02d}_release.json",{"status":"decode_failed"})
        return 1
    finally:
