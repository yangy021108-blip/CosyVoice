#!/usr/bin/env python3
"""Ready PD prefill worker for interference measurements."""
...
def parse_args():
    ...
    p.add_argument("--b-prompt-length", type=int, required=True)
    p.add_argument("--side-channel-port", type=int, required=True)
    p.add_argument("--timeout", type=float, default=600)
    p.add_argument("--instrument-nixl", action="store_true")
    p.add_argument("--standard-optimized", action="store_true")
    return p.parse_args()
...
class GpuSampler:
    ...
    def _run(self):
        ...
        try:
            pynvml.nvmlInit()
            handle = pynvml.nvmlDeviceGetHandleByIndex(int(self.gpu))
        except Exception:
            handle = None
        while not self.stop_event.is_set():
            ...
            if handle is not None:
                try:
                    ...
                    self.samples.append({"monotonic_ns": now,
    ...
    def summary(self, start_ns, end_ns):
        selected=[x for x in self.samples if start_ns <= x["monotonic_ns"] <= end_ns]
        vals=[x["utilization_gpu_percent"] for x in selected]
        return {"sample_count":len(selected), "mean_percent":(sum(vals)/len(vals) if vals else None),
                "max_percent":(max(vals) if vals else None), "samples":selected}
...
def main():
    ...
    if a.instrument_nixl:
        ...
        os.environ["COSY_PD_TRACE_FILE"]=str(a.work/"prefill_trace.jsonl")
    ...
    try:
        ...
        def run_prefill(name, req_id, p):
            params={"do_remote_decode":True,"do_remote_prefill":False,
            ...
            tokens, metrics, transfer=run_engine_request(
                ...
                request_id=req_id, timeout_seconds=a.timeout, nvtx_label="COSY_PD_INTERFERENCE_PREFILL_"+name.upper())
            finished=time.perf_counter_ns()
            if not transfer: raise RuntimeError("missing KV transfer metadata")
            write_json(a.work/f"{name}_prefill.json", {"status":"ok","request_id":req_id,
        ...
        run_prefill("warm", "pd-p-warm", prompt)
        wait_for_file(a.work/"warm_release.json", a.timeout)
        run_prefill("control", "pd-p-control", prompt)
        wait_for_file(a.work/"control_release.json", a.timeout)
        run_prefill("measured", "pd-p-measured", prompt)
        wait_for_file(a.work/"barrier.json", a.timeout)
        ...
        btokens,bmetrics,_=run_engine_request(
            engine,bprompt,make_sampling_params(metadata,prefill_only=True),
            request_id="pd-p-b",timeout_seconds=a.timeout,nvtx_label="COSY_PD_INTERFERENCE_B_PREFILL")
        b_finished=time.perf_counter_ns(); sampler.stop()
        write_json(a.work/"b_prefill.json", {"status":"ok","request_id":"pd-p-b",
            "prompt_tokens":a.b_prompt_length,"started_monotonic_ns":b_started,
            "finished_monotonic_ns":b_finished,"auxiliary_tokens":btokens,
            "metrics":bmetrics,"gpu_utilization":sampler.summary(b_started,b_finished)})
        wait_for_file(a.work/"measured_release.json", a.timeout)
        return 0
    except Exception as exc:
        err={"status":"error","stage":"prefill","exception":f"{type(exc).__name__}: {exc}",
             "traceback":traceback.format_exc()}
        for name in ("warm_prefill","control_prefill","measured_prefill","b_prefill"):
